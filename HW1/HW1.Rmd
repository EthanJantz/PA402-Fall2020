...
In the following chunk I will be removing observations where the variables can't be used for analysis and outputting a new frequency table for each variable. 
...
selected <- selected %>%
  filter(
    FamIncome != 1 # !# is a unary operator that means "not". So you can read this as "Only keep the rows where FamIncome is NOT 1"
  )
```
...
And since we would likely be using this dataset, let's export it too.

```{r export, message = FALSE}
write.csv("ANES_selected.csv") # Export the final dataset to the project folder as .csv
# Ignore the extra text below this. I'm not sure how to remove it.
```